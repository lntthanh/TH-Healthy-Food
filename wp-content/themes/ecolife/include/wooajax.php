<?php
//WooCommerce Ajax
add_action('wp_head','ecolife_woo_ajaxurl');
function ecolife_woo_ajaxurl() {
?>
<?php
	...
	<div class="woocommerce product">
		...
		<div class="product-info">
			...
			<div class="price-box" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
				<p class="price"><?php printf( '%s', $product->get_price_html() ); ?></p>
			</div>
			<a class="see-all" href="<?php echo esc_url($product->get_permalink()); ?>"><?php echo esc_html($ecolife_opt['quickview_link_text']); ?></a>
			<div class="quick-add-to-cart">
				<?php woocommerce_template_single_add_to_cart(); ?>
			</div>
			<div class="quick-desc"><?php echo do_shortcode(get_post($productid)->post_excerpt); ?></div>
			<?php if( function_exists('ecolife_product_sharing') ) { ?>
				<div class="social-sharing"><?php ecolife_product_sharing(); ?></div>
			<?php } ?>
		</div>
	</div>
	<?php
...
function get_cartinfo() {
	global $woocommerce;
	echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'ecolife'), $woocommerce->cart->cart_contents_count);
...
function get_productinfo() {
	...
	$product = get_product( $productid );
	?>
	<h3><?php esc_html_e('Product is added to cart', 'ecolife');?></h3>
	...
	<div class="buttons">
		<a class="button" href="<?php echo esc_url(get_permalink( wc_get_page_id( 'cart' ) )); ?>"><?php esc_html_e('View Cart', 'ecolife');?></a>
	</div>
	<?php
	die();
}