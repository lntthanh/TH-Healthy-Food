...
//			$check = $wpdb->query("DELETE FROM `".$wpdb->prefix."posts` WHERE `post_type`='nav_menu_item'");
		...
		function download_url( $url, $timeout = 300 ) {
			  ...
			  if ( ! $url )
				return new WP_Error('http_no_url', __('Invalid URL Provided.','ecolife'));

			  $url_filename = basename( parse_url( $url, PHP_URL_PATH ) );

			  $tmpfname = wp_tempnam( $url_filename );
			  if ( ! $tmpfname )
				return new WP_Error('http_no_file', __('Could not create Temporary file.','ecolife'));
		...
		function import_start( $file ) {
			...
			if ( is_wp_error( $import_data ) ) {
				echo '<p><strong>' . __( 'Sorry, there has been an error.', 'ecolife' ) . '</strong><br />';
		...
		function handle_upload() {
			...
			if ( isset( $file['error'] ) ) {
				...
				echo '<p><strong>' . __( 'Sorry, there has been an error.', 'ecolife' ) . '</strong><br />';
				printf( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'ecolife' ),  $file['file'] );
				echo '</p>';
				return false;
			}
			...
			if ( $this->version > $this->max_wxr_version ) {
				...
				printf( __( 'This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'ecolife' ), esc_html($import_data['version']) );
				echo '</strong></p></div>';
			}
	...
	<?php if ( ! empty( $this->authors ) ) : ?>
		<h3><?php _e( 'Assign Authors', 'ecolife' ); ?></h3>
		<p><?php _e( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', 'ecolife' ); ?></p>
	<?php if ( $this->allow_create_users() ) : ?>
		<p><?php printf( __( 'If a new user is created by WordPress, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.', 'ecolife' ), esc_html( get_option('default_role') ) ); ?></p>
	<?php endif; ?>
		<ol id="authors">
	...
	<?php if ( $this->allow_fetch_attachments() ) : ?>
		<h3><?php _e( 'Import Attachments', 'ecolife' ); ?></h3>
		<p>
			<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
			<label for="import-attachments"><?php _e( 'Download and import file attachments', 'ecolife' ); ?></label>
		</p>
	<?php endif; ?>
	
		<p class="submit"><input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'ecolife' ); ?>" /></p>
	</form>
	<?php
		}
		...
		if ( $create_users ) {
			if ( $this->version != '1.0' ) {
				_e( 'or create new user with login name:', 'ecolife' );
				$value = '';
			} else {
				_e( 'as a new user:', 'ecolife' );
		...
		if ( ! $create_users && $this->version == '1.0' )
			_e( 'assign posts to an existing user:', 'ecolife' );
		else
			_e( 'or assign posts to an existing user:', 'ecolife' );
		wp_dropdown_users( array( 'name' => "user_map[$n]", 'multi' => true, 'show_option_all' => __( '- Select -', 'ecolife' ) ) );
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'ecolife' ), esc_html($this->authors[$old_login]['author_display_name']) );
	...
	function process_terms() {
		...
		if ( ! $menu_slug ) {
			//echo "<pre>"; print_r($item['terms']); echo "</pre>";
			//_e( 'Menu item skipped due to missing menu slug', 'ecolife' );
			//echo '<br />';
			return;
		}
		...
		if ( ! $menu_id ) {
			printf( __( 'Menu item skipped due to invalid menu slug: %s', 'ecolife' ), esc_html( $menu_slug ) );
			echo '<br />';
			return;
		} else {
		...
		function process_attachment( $post, $url ) {
			...
			if ( ! $this->fetch_attachments )
				return new WP_Error( 'attachment_processing_error',
					__( 'Fetching attachments is not enabled', 'ecolife' ) );
			...
			if ( $info = wp_check_filetype( $upload['file'] ) )
				...
				return new WP_Error( 'attachment_processing_error', __('Invalid file type', 'ecolife') );
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( ! $headers ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', __('Remote server did not respond', 'ecolife') );
			} 
			// make sure the fetch was successful
			if ( $headers['response'] != '200' ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', sprintf( __('Remote server returned error response %1$d %2$s', 'ecolife'), esc_html($headers['response']), get_status_header_desc($headers['response']) ) );
			}
			...
			if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', __('Remote file is incorrect size', 'ecolife') );
			}
	
			if ( 0 == $filesize ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', __('Zero size file downloaded', 'ecolife') );
			}
	
			$max_size = (int) $this->max_attachment_size();
			if ( ! empty( $max_size ) && $filesize > $max_size ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', sprintf(__('Remote file is too large, limit is %s', 'ecolife'), size_format($max_size) ) );
			}
		...
		function header() {
			echo '<div class="wrap">';
			echo '<h2>' . __( 'Import WordPress', 'ecolife' ) . '</h2>';
			...
			if ( isset( $updates[$basename] ) ) {
				...
				echo '<div class="error"><p><strong>';
				printf( __( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'ecolife' ), $update->update->new_version );
				echo '</strong></p></div>';
			}
		}
		...
		function greet() {
			...
			echo '<p>'.__( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'ecolife' ).'</p>';
			echo '<p>'.__( 'Choose a WXR (.xml) file to upload, then click Upload file and import.', 'ecolife' ).'</p>';
...
if( !empty( $_POST['importSampleData'] ) ){ 
	...
	//	$file = dirname( __FILE__ ) .DS.'data'.DS.'all-content.xml';
	$import_data = null; 
	...
	if(isset($_POST["active_demo"]) && $_POST["active_demo"] == 2) {
		...
		// set theme options 
		$json_file = THEME_DIRECTORY.'/road_importdata/data/'.$import_data.'/theme_options.json';
		$devnImperter->import_option_data($opt_name,$json_file,'theme_option');

		//set menu theme options 
		$menu_file = THEME_DIRECTORY.'/road_importdata/data/'.$import_data.'/mega-main-menu-backup.txt';
		$devnImperter->import_option_data('mega_main_menu_options',$menu_file,'menu'); 

		$url = admin_url('admin.php?page=roadtheme-setup');
		wp_redirect($url); 
		exit;
		return ; 
	}
	$road_theme_dir = get_template_directory();
	
	$file = $road_theme_dir.'/road_importdata/data/'.$import_data.'/all_content.xml';
	
	// url download media from your server 
		$url = 'http://'.API_SERVER.'/demo/ecolife/wp-content/uploads/uploads.zip';
		if (isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] == 'https') {
			$url = 'https://'.API_SERVER.'/demo/ecolife/wp-content/uploads/uploads.zip';
		}
