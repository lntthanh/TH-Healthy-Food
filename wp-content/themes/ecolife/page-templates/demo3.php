...
if(count($jscomposer_templates) > 0) {
	foreach($jscomposer_templates as $jscomposer_template){
		if($jscomposer_template->post_title == 'Header3' || $jscomposer_template->post_title == 'Footer1'){
...
<body <?php body_class('home'); ?>>
	...
	<div class="wrapper">
		<div class="page-wrapper">
			<div class="header-container header3 rs-active has-mobile-layout">  
			 	<div class="header">
					<div class="header-content">
						...
						if ( isset($ecolife_opt['header_layout']) && $ecolife_opt['header_layout']!="") {
							...
							if(count($jscomposer_templates) > 0) {
								foreach($jscomposer_templates as $jscomposer_template){
									if($jscomposer_template->post_title == 'Header3'){ ?>
							...
							if ( isset($ecolife_opt['header_mobile_layout']) && $ecolife_opt['header_mobile_layout'] != "") {
								if(count($jscomposer_templates) > 0) {
									foreach($jscomposer_templates as $jscomposer_template){
										if($jscomposer_template->post_title == 'HeaderMobile3'){ ?>
							...
							if ( isset($ecolife_opt['sticky_header']) && $ecolife_opt['sticky_header']) {
								if(count($jscomposer_templates) > 0) {
									foreach($jscomposer_templates as $jscomposer_template){
										if($jscomposer_template->post_title == 'HeaderSticky3'){ ?>
					...
					<div class="footer-inner">
						...
						if ( isset($ecolife_opt['footer_layout']) && $ecolife_opt['footer_layout']!="" ) { ?>
							<div class="footer-composer">
								...
								if(count($jscomposer_templates) > 0) {
									foreach($jscomposer_templates as $jscomposer_template){
										if($jscomposer_template->post_title == 'Footer1'){
											echo do_shortcode(apply_filters( 'the_content', $jscomposer_template->post_content ));
										}
									}
								} ?>
