...
class RoadProducts {
	...
	public static function testimonial_r ( $atts, $content = null ) {
		...
		if ($atts["items_993_1200"] == '' ) {$atts["items_993_1200"] = 1; }
		if ($atts["items_769_992"]  == '' ) {$atts["items_769_992"]  = 1; }
		if ($atts["items_641_768"]  == '' ) {$atts["items_641_768"]  = 1; }
		if ($atts["items_360_640"]  == '' ) {$atts["items_360_640"]  = 1; }
		...
		$navigation = 1;
		$navigation = ($atts["navigation"] == 0) ? 0 : 1;

		$pagination = 0;
		$pagination = ($atts["pagination"] == 1) ? 1 : 0;
		...
		$thumbnail = '';
		$thumbnail = ($atts["thumbnail"] == true) ? ' has-thumb ' : '';
		...
		return '<div class="testimonial_r'. $thumbnail .'" data-margin='.$margin.' data-1201up='.$args["items_1201up"].' data-993-1200='.$args["items_993_1200"].' data-769-992='.$args["items_769_992"].' data-641-768='.$args["items_641_768"].' data-360-640='.$args["items_360_640"].' data-0-359='.$args["items_0_359"].' data-navigation='.$navigation.' data-pagination='.$pagination.' data-speed='.$atts["speed"].' data-loop='.$loop.' data-auto='.$auto.'>'.woothemes_testimonials( $args ).'</div>' ;
	...
	public function road_vc_shortcodes() {
		...
		$order_by_values = array(
			'',
			__( 'Date', 'ecolife' )          => 'date',
			__( 'ID', 'ecolife' )            => 'ID',
			__( 'Author', 'ecolife' )        => 'author',
			__( 'Title', 'ecolife' )         => 'title',
			__( 'Modified', 'ecolife' )      => 'modified',
			__( 'Random', 'ecolife' )        => 'rand',
			__( 'Comment count', 'ecolife' ) => 'comment_count',
			__( 'Menu order', 'ecolife' )    => 'menu_order',
		);

		$order_way_values = array(
			'',
			__( 'Descending', 'ecolife' ) => 'DESC',
			__( 'Ascending', 'ecolife' )  => 'ASC',
		); 
	 
	//Sale products Roadthemes
	vc_map( array(
		'name'        => __( 'RT Sale products', 'ecolife' ),
		'base'        => 'sale_products_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'ecolife' ),
		'description' => __( 'List all products on sale', 'ecolife' ),
		'params'      => array_merge(
			array(
				array(
					'type'        => 'textfield',
					'heading'     => esc_html__( 'Per page', 'ecolife' ),
					'value'       => 12,
					'save_always' => true,
					'param_name'  => 'per_page',
					'description' => esc_html__( 'How much items per page to show', 'ecolife' ),
				),
					array(
					'type'        => 'textfield',
					'heading'     => __( 'Columns', 'ecolife' ),
					'value'       => 4,
					'save_always' => true,
					'param_name'  => 'columns',
					'description' => __( 'How much columns grid', 'ecolife' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Order by', 'ecolife' ),
					'param_name'  => 'orderby',
					'value'       => $order_by_values,
					'save_always' => true,
					'description' => sprintf( __( 'Select how to sort retrieved products. More at %s.', 'ecolife' ), '<a href="http://codex.wordpress.org/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">WordPress codex page</a>' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Sort order', 'ecolife' ),
					'param_name'  => 'order',
					'value'       => $order_way_values,
					'save_always' => true,
					'description' => sprintf( __( 'Designates the ascending or descending order. More at %s.', 'ecolife' ), '<a href="http://codex.wordpress.org/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">WordPress codex page</a>' ),
				), 
	...
	vc_map( array(
		'name'        => __( 'RT Featured products', 'ecolife' ),
		'base'        => 'featured_products_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'ecolife' ),
		'description' => __( 'Display products set as "featured"', 'ecolife' ),
		'params'      => array_merge(
		array(
			array(
				'type'        => 'textfield',
	...
	vc_map( array(
	'name'        => __( 'RT Recent products', 'ecolife' ),
	'base'        => 'recent_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'ecolife' ),
	'description' => __( 'Lists recent products', 'ecolife' ),
	'params'      => array_merge( 
		array(
			array(
				'type'        => 'textfield',
	...
	vc_map( array(
	'name' => __( 'Top Rated Products', 'ecolife' ),
	'base' => 'top_rated_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category' => __( 'Theme', 'ecolife' ),
	'description' => __( 'List all top rated products', 'ecolife' ),
	'params'      => array_merge( 
		array(
			array(
				'type' => 'textfield',
				'heading' => __( 'Per page', 'ecolife' ),
				'value' => 12,
				'param_name' => 'per_page',
				'save_always' => true,
				'description' => __( 'How much items per page to show', 'ecolife' ),
			),
			array(
				'type' => 'textfield',
				'heading' => __( 'Columns', 'ecolife' ),
	...
	vc_map( array(
	'name'        => __( 'RT Bestselling products', 'ecolife' ),
	'base'        => 'bestselling_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'ecolife' ),
	'description' => __( 'List best selling products on sale', 'ecolife' ),
	'params'      => array_merge( 
		array(
				array(
					'type'        => 'textfield',
	...
	//products Roadthemes
	vc_map( array(
	'name'        => __( 'RT Products', 'ecolife' ),
	'base'        => 'list_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'ecolife' ),
	'description' => __( 'Show multiple products by ID or SKU.', 'ecolife' ),
	'params'      => array_merge(
		array(
			array(
				'type'        => 'textfield',
			...
			array(
				...
				'save_always' => true,
				'description' => __( 'Enter List of Products', 'ecolife' ),
			),
			array(
				'type'       => 'hidden',
				'param_name' => 'skus',
	...
	vc_map( array(
	'name'        => __( 'RT Product categories', 'ecolife' ),
	'base'        => 'product_categories_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'ecolife' ),
	'description' => __( 'Display product categories loop', 'ecolife' ),
	'params'      => array_merge( 
			array(
				array(
					'type'        => 'textfield',
					'heading'     => __( 'Number', 'ecolife' ),
					'param_name'  => 'number',
					'description' => __( 'The `number` field is used to display the number of products.', 'ecolife' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Order by', 'ecolife' ),
				...
				array(
					...
					'description' => __( 'Hide empty', 'ecolife' ),
				),
				array(
					'type'       => 'autocomplete',
					'heading'    => __( 'Categories', 'ecolife' ),
					'param_name' => 'ids',
					'settings'   => array(
						'multiple' => true,
						'sortable' => true,
					...
					'save_always' => true,
					'description' => __( 'List of product categories', 'ecolife' ),
				), 
			),road_get_slider_setting()
		)
	)  );
		...
		vc_map( array(
		'name'        => __( 'RT Product category', 'ecolife' ),
		'base'        => 'product_category_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'ecolife' ),
		'description' => __( 'Show multiple products in a category', 'ecolife' ),
		'params'      => array_merge(
			array(
				array(
					'type'        => 'textfield',
				...
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Category', 'ecolife' ),
					'value'       => $product_categories_dropdown,
					'param_name'  => 'category',
					'save_always' => true,
					'description' => __( 'Product category list', 'ecolife' ),
				),
			),road_get_slider_setting()
		)
	)  );
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $product_object ) ) {
				...
				if ( ! empty( $product_sku ) ) {
					$product_sku_display = ' - ' . __( 'Sku', 'ecolife' ) . ': ' . $product_sku;
				}

				$product_title_display = '';
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . __( 'Title', 'ecolife' ) . ': ' . $product_title;
				}

				$product_id_display = __( 'Id', 'ecolife' ) . ': ' . $product_id;
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = __( 'Id', 'ecolife' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . __( 'Title', 'ecolife' ) . ': ' . $value['title'] : '' ) . ( ( strlen( $value['sku'] ) > 0 ) ? ' - ' . __( 'Sku', 'ecolife' ) . ': ' . $value['sku'] : '' );
				$results[] = $data;
			}
		}
	...
	public function productCategoryCategoryAutocompleteSuggester( $query, $slug = false ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = __( 'Id', 'ecolife' ) . ': ' . $value['id'] . ( ( strlen( $value['name'] ) > 0 ) ? ' - ' . __( 'Name', 'ecolife' ) . ': ' . $value['name'] : '' ) . ( ( strlen( $value['slug'] ) > 0 ) ? ' - ' . __( 'Slug', 'ecolife' ) . ': ' . $value['slug'] : '' );
				$result[] = $data;
			}
		}
	...
	protected function productCategoryTermOutput( $term ) {
		...
		$term_slug_display = '';
		if ( ! empty( $term_slug ) ) {
			$term_slug_display = ' - ' . __( 'Sku', 'ecolife' ) . ': ' . $term_slug;
		}

		$term_title_display = '';
		if ( ! empty( $term_title ) ) {
			$term_title_display = ' - ' . __( 'Title', 'ecolife' ) . ': ' . $term_title;
		}

		$term_id_display = __( 'Id', 'ecolife' ) . ': ' . $term_id;
	...
	public static function getProductsFieldsList() {
		return array(
			__( 'SKU', 'ecolife' )               => 'sku',
			__( 'ID', 'ecolife' )                => 'id',
			__( 'Price', 'ecolife' )             => 'price',
			__( 'Regular Price', 'ecolife' )     => 'regular_price',
			__( 'Sale Price', 'ecolife' )        => 'sale_price',
			__( 'Price html', 'ecolife' )        => 'price_html',
			__( 'Reviews count', 'ecolife' )     => 'reviews_count',
			__( 'Short description', 'ecolife' ) => 'short_description',
			__( 'Dimensions', 'ecolife' )        => 'dimensions',
			__( 'Rating count', 'ecolife' )      => 'rating_count',
			__( 'Weight', 'ecolife' )            => 'weight',
			__( 'Is on sale', 'ecolife' )        => 'on_sale',
			__( 'Custom field', 'ecolife' )      => '_custom_',
		);
	}
	...
	public static function getOrderFieldsList() {
		return array(
			__( 'ID', 'ecolife' )                       => 'id',
			__( 'Order number', 'ecolife' )             => 'order_number',
			__( 'Currency', 'ecolife' )                 => 'order_currency',
			__( 'Total', 'ecolife' )                    => 'total',
			__( 'Status', 'ecolife' )                   => 'status',
			__( 'Payment method', 'ecolife' )           => 'payment_method',
			__( 'Billing address city', 'ecolife' )     => 'billing_address_city',
			__( 'Billing address country', 'ecolife' )  => 'billing_address_country',
			__( 'Shipping address city', 'ecolife' )    => 'shipping_address_city',
			__( 'Shipping address country', 'ecolife' ) => 'shipping_address_country',
			__( 'Customer Note', 'ecolife' )            => 'customer_note',
			__( 'Customer API', 'ecolife' )             => 'customer_api',
			__( 'Custom field', 'ecolife' )             => '_custom_',
		);
	}
