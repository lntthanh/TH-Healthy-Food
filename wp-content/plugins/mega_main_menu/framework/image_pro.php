	...
	if ( !class_exists( 'mm_image_pro' ) ) {
		...
		*/
			...
			public static function processed_image ( $args = array() ) {
				...
				// build image tag
				if ( $src !== false ) {
					$img = '<img src="' . self::resized_image_src( $src, $width, $height, $crop ) . '" alt="' . $alt_attr . '" title="' . esc_url( $post_excerpt ) . '" />';
...
/*
								...
								$out .= mm_common::ntab(3) . '<a ' . $link_href_atr . ' class="title' . ( $permalink == '' ? ' single' : '' ) . '" title="' . esc_url( $title ) . '">';
								$out .= mm_common::ntab(4) . $title;					
								$out .= mm_common::ntab(3) . '</a>';
*/
								$out .= mm_common::ntab(3) . ( ( $permalink != '' && ( is_array( $cover ) && !in_array( 'link', $cover ) ) ) 
									? '<a href="' . $permalink . '"' . ' class="title' . ( $permalink == '' ? ' single' : '' ) . '" title="' . esc_url( $title ) . '"' . ( ( $custom_color_2 != false ) ? ' style="background: ' . $custom_color_2 . ';"' : '' ) . '>'. $title . '</a>'
							...
							if ( $cover == 'zoom' || ( is_array( $cover ) && in_array( 'zoom', $cover ) ) ) {
								$out .= mm_common::ntab(3) . '<a href="' . $src . '" title="' . esc_url( $title ) . '" data-rel="prettyPhoto' . ( ( $stack_id !== false ) ? '[pp_' . $stack_id . ']' : '' ) . '" class="controls full_image' . ( ( $permalink != '' && in_array( 'link', $cover ) ) ? '' : ' single' ) . '">';
